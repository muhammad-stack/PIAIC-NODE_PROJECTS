...
async function BankServices(bank: Bank) {
  ...
  if (services.Services == "Cash WithDraw") {
    let res = await inquirer.prompt({
      ...
      validate: (input) => {
        if (isNaN(input)) {
            return `Please Enter Your Account ${chalk.red.bold.visible(
                "Number"
                )}`;
            } else {
                return true;
    ...
    let account = myBank.accounts.find(
      (account) => account.accountNo == res.Num
    );
    if (!account) {
    ...
    let account = myBank.accounts.find(
        ...
        if (account) {
            let name = myBank.customers.find(
                (name) => name.AccountNo == account?.accountNo
                );
                console.log(` Dear ${chalk.green.bold(name?.Fname)} ${chalk.green.bold(
                    name?.Lname
                    )}
